...
| **P3 — Sales handoff** ✅ | `SALES` role + introduction page + first-service cues on the prep sheet | migration 0029: enum values, `introduced_advisor_id`, `sold_by_user_id`, `visit_context`, `owning_advisor_*` | **BUILT** `54e7e47` (Opus, Fable-verified). P2's owning seam is live; FIRST_VISIT ownership hooks in `closeRepairOrder`; Q2 honoured as recommended (first services only). **Two plan corrections found:** D5's "deny-by-default already protects every other route" was false — deny-by-default is authentication, not role, so the 15-page `fenceSales` guard was real unassumed work; and `maintenanceSchedules` "knows the interval" overstated an empty, unread, ungranted table (the sixth policy-without-grant, fixed in 0029) — every store today uses the stated 6mo/5k fallback. MANAGER_SET/REQUESTED owning sources are vocabulary only (no manager edit surface). **0029 apply-before-deploy: breaks reads on customers and appointments** |
| **P4 — Timeline** ✅ | `src/lib/timeline/` + customer/vehicle rendering + prep-sheet card; fix `CALL_ME` outcome vocabulary; first consumer for `needsFollowUp` | migration 0030: the `CALL_ME` enum value **and** the seventh policy-without-grant (`presentation_sessions`) + four restated grants | **BUILT** `db61b74` (Opus, Fable-verified). Every aggregation consumer treats `CALL_ME` deliberately; no backfill possible (stored call-mes are indistinguishable `SKIPPED` rows, stated in the migration). Constant-cost timeline (14 queries regardless of customer age); open threads is `needsFollowUp`'s first consumer, call-me leads everything and never collapses into a decline. **0030 apply-before-deploy: the grant breaks reads loudly, the enum value breaks writes quietly** — an un-migrated store records zero outcomes for any visit containing a call-me. **Flagged residual:** `api/copilot/route.ts` still drops `CALL_ME` from its hand-written decision list (F5's bug one surface over) — one-token fix, outside P4's scope |

**All four phases are built.** The plan is delivered end to end as of
2026-08-18; the ship gate is `npm run db:apply` (0028, 0029, 0030) before the
push that deploys any of it. Still open: the §9 product questions not yet
answered (Q1 and Q2 shipped on their recommended defaults, commented; Q4, Q5
pending), the copilot residual above, and the pre-existing seams recorded on
the P2/P3 rows (shifts, timezone, advisor-id mapping, manager owning-advisor
surface).
